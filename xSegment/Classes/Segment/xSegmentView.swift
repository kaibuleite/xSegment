...
public class xSegmentView: xView {
    ...
    public func updateItemStyleToNormal(at idx : Int)
    {
        ...
        self.updateItemTitleColor(at: idx, color: cfg.titleColor.normal)
    }
    /// 设置选中样式
    public func updateItemStyleToChoose(at idx : Int)
    {
        ...
        self.updateItemTitleColor(at: idx, color: cfg.titleColor.choose)
    }
    ...
    ///   - idx: 编号
    ///   - color: 颜色
    public func updateItemTitleColor(at idx : Int,
                                     color : UIColor)
    {
        guard let item = self.getItem(at: idx)  else { return }
    ...
    /// 设置指示线位置（直接跳过去）
    public func updateLineLocation(at idx : Int)
    {
        guard let item = self.getItem(at: idx)  else { return }
        let cfg = self.config
        ...
        pos.x += lineW      // 终点位置
        path.addLine(to: pos)
        self.updateLineLayer(path: path, animate: false)
    }
    
    /// 设置指示线路径
    public func updateLineLayer(path : UIBezierPath,
                                animate : Bool)
    {
        guard animate else {
    ...
    /// 设置滚动结果位置（边缘处理）
    public func updateScrollContentSide(at idx : Int)
    {
